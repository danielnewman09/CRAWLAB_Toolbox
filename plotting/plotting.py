...
def generate_plot(
    ...
    # Show the legend
    ax.legend(ncol=num_col,loc=legend_loc).get_frame().set_edgecolor('k')
...
def plot_3d(
    ...
    showplt=False,folder='Figures/Miscellaneous',
    filename='3d_plot',
    xticks=1,yticks=1,zticks=1,
    enablelog=False,labelsize=24,labelpad=20,
    rotated=False,transparent=False):
    '''
   ...
   Creator : Daniel Newman - Danielnewman09@gmail.com
    ...
    ax1.set_zlabel(zlabel,family='serif',fontsize=labelsize,labelpad=10,rotation=90)
    
    if showplt:
        plt.show()    
    else:
        # Save the pdf of the plot    
        if transparent and len(filename) > 0:
            plt.savefig('{}/{}.png'\
                    .format(folder,filename),transparent=transparent)
        # Save the pdf of the plot    
        elif len(filename) > 0:
            plt.savefig('{}/{}.pdf'\
                    .format(folder,filename))     
        else:
            plt.savefig('{}/Misc.pdf'\
                    .format(folder))
